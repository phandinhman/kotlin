...
class SerializeRecTest {
    ...
    companion object {
        ...
        fun checkDataDesc(desc: KSerialClassDesc) {
            if (desc.name != "test.serialization.Data") fail("checkDataDesc name $desc")
    ...
    class Out() : ElementValueOutput() {
        ...
        override fun writeBegin(desc: KSerialClassDesc) {
            when(step) {
                ...
                3 -> { checkDataDesc(desc); step++; return }
            }
        ...
        override fun writeElement(desc: KSerialClassDesc, index: Int) {
            when (step) {
                1 -> { checkContainerDesc(desc); if (index == 0) { step++; return } }
                4 -> { checkDataDesc(desc); if (index == 0) { step++; return } }
                6 -> { checkDataDesc(desc); if (index == 1) { step++; return } }
            }
